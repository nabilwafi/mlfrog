...
class RandomForestTrainer(BaseTrainer):
    ...
    def fit(self, train: Dataset, validation: Dataset, *, context: dict[str, Any]) -> Model:
        ...
        clf = RandomForestClassifier(
            ...
            class_weight="balanced_subsample" if self.params.get("class_weight", True) else None,
            random_state=seed,
        ...
        try:
            ...
            raise TrainerError(f"random forest failed: {exc}") from exc

        importance = {
            n: float(v) for n, v in zip(feature_names, clf.feature_importances_, strict=True)
        }
        return Model(
            symbol=train.symbol,
            ...
            metadata={"best_iteration": 0, "target_mode": target_mode, "n_estimators": clf.n_estimators},
            estimator=clf,
        )
