"""Walk-forward runner with gain/split/SHAP importance for structure cols."""
...
class SelectionWalkForwardRunner:
    def __init__(
        self,
        *,
    ...
    def run(
        ...
        *,
        ...
        struct_set = set(structure_feature_names) & set(feature_names)
        ...
        for window in self.windows:
            ...
            context = {
                ...
                "dataset_version": "structure_selection_v1",
            }
            ...
            gain, split = self._importance_maps(model, struct_set)
            shap_map = self._shap_map(model, x_val, struct_set) if self.compute_shap else {}
            ...
            logger.info(
                "StructureSelection %s/%s | %s roc=%.4f",
                side,
    ...
    @staticmethod
    def _importance_maps(model: Any, struct_set: set[str]) -> tuple[dict[str, float], dict[str, float]]:
        gain = {
            k: float(v)
        ...
        booster = getattr(model, "estimator", None)
        if booster is not None and hasattr(booster, "feature_importance"):
            try:
                names = list(model.feature_names)
                vals = booster.feature_importance(importance_type="split")
                split = {
                    n: float(v)
    ...
    @staticmethod
    def _shap_map(model: Any, x_val: pd.DataFrame, struct_set: set[str]) -> dict[str, float]:
        booster = getattr(model, "estimator", None)
        ...
        try:
            ...
            if arr.ndim != 2 or arr.shape[1] < 2:
                return {}
            names = list(model.feature_names)
            means = np.abs(arr[:, :-1]).mean(axis=0)
            return {
                n: float(m)
                for n, m in zip(names, means, strict=True)
                if n in struct_set
            }
        except Exception:
            return {}
