"""CLI: Probability quality analysis for Long/Short model v2 (Sprint 14).
...
def _setup_logging(cfg: dict[str, Any]) -> None:
    ...
    fh = RotatingFileHandler(
        log_dir / str(log_cfg.get("filename", "probability_quality.log")),
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Probability quality analysis for model v2")
...
def main(argv: list[str] | None = None) -> int:
    ...
    timeframe = args.timeframe or str(pq_cfg.get("timeframe") or mv_cfg.get("timeframe") or "H1")

    sides_raw = args.sides or pq_cfg.get("sides") or mv_cfg.get("sides") or "long,short"
    ...
    algorithm = str(
        pq_cfg.get("algorithm") or mv_cfg.get("algorithm") or models_cfg.get("algorithm") or "lightgbm"
    )
    algo_params = dict((models_cfg.get("algorithms") or {}).get(algorithm) or {})
    trainer_params = dict(
        pq_cfg.get("trainer_params")
    ...
    merged = {
        ...
        "random_seed": int(
            pq_cfg.get("random_seed", mv_cfg.get("random_seed", research_cfg.get("random_seed", 42)))
        ),
        "windows": pq_cfg.get("windows") or mv_cfg.get("windows") or research_cfg.get("windows"),
    }
    ...
    out_root = _resolve_path(
        str(pq_cfg.get("output_directory", RESEARCH / "probability_quality"))
    )
    ...
    preds, out = RunProbabilityQualityUseCase(
        ProbabilityQualityRepository(out_root),
        config=merged,
    ).execute(
        symbol=symbol,
        timeframe=timeframe,
    ...
    if not preds.empty:
        for side in sides:
            ...
            if sub.empty:
                continue
            print(
                f"{side}: n={len(sub)} mean_p={sub['y_prob'].mean():.4f} "
                f"win_rate={sub['y_true'].mean():.4f}"
            )
    print(f"saved -> {out}")
    return 0
